...
try {  
  ...
  if (err.message.includes("not authorized")) {  
    reply(`❌ Couldn't make the bot admin.\n\n✳️ *Reason:* You don't have permission to promote the bot.\n\n✳️ *Manual steps:*\n1. Open group settings\n2. Go to "Group permissions"\n3. Go to "Add members"\n4. Find the bot and promote it manually`);  
  } else {  
...
// If a number was given (extract from text)  
if (users.length === 0 && text) {  
  ...
  if (directNumbers) {  
    users = directNumbers.map(num => {  
      // Add +92 / 92 for Pakistan numbers  
...
// Fallback: extract from the message body  
